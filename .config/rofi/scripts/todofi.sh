...
if [[ -z "$TODO_BIN" ]]; then
    echo "Missing todo.sh, please install it !"
    exit 1
fi

runrofi () {
    $ROFI_BIN -theme mytheme -width 50% -matching glob -tokenize -i -no-levenshtein-sort "$@"
}

runtodo_verbose() {
    todo.sh -p -d "$TODOTXT_CFG_FILE" "$@"
}
...
confirm() {
    ...
    if [[ $context ]]; then
        ...
        response=$(echo -e "Yes\nNo" | $ROFI_BIN -theme mytheme -lines 2 -u 0 -a 1 -dmenu -mesg "$mesg" -i -p "$message")
    else
        response=$(echo -e "Yes\nNo" | $ROFI_BIN -theme mytheme -lines 2 -u 0 -a 1 -dmenu -i -p "$message")
    fi
...
Todo.txt format: https://github.com/todotxt/todo.txt"
    ...
    if [[ $val -eq 0 ]]; then
        ...
        if [[ "$result" ]]; then
            $ROFI_BIN -theme mytheme -e "$result"
        fi
    fi
}
